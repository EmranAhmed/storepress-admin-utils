...
if ( ! class_exists( '\StorePress\AdminUtils\REST_API' ) ) {
	class REST_API extends \WP_REST_Controller {
		...
		public function __construct( Settings $settings ) {
			...
			$this->namespace = sprintf( '%s/v1', $this->get_settings()->get_page_id() );
		}
		 ...
		 * @return self
		 */
		public function add_namespace( string $namespace ): REST_API {
		 ...
		 * @return self
		 */
		public function add_capability( string $permission ): REST_API {
		...
		public function register_routes() {
			...
			/*
				print_r( $this->get_public_item_schema() );
			die;*/
		...
		public function get_item( $request ) {
			...
			foreach ( $options as $name => $args ) {
				 ...
				 */
				$response[ $name ] = apply_filters( 'rest_pre_get_storepress_setting', null, $name, $args );
